...
Для сохранения данный в сессию пользователя (таких, как идентификатора локализации `lang = ua`, либо признака возможности редактирования содержимого `admin = true`) необходимо выполнить функцию `session()` с двумя аргументами:
