...
```javascript
preedit(node) {
	console.log("Сейчас будет отредактирован блок " + node);
}

postedit(node) {
	console.log("Только что был отредактирован блок " + node);
}
```
...
Sitar вызовет эти функции в соответствующей ситуации, если они были определены, и передаст в них единственный аргумент `node` — элемент html, с которым проводится манипуляция.
